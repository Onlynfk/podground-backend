...
class PostService:
    ...
    async def get_blog_categories(self) -> List[BlogCategory]:
        ...
        try:
            ...
            result = self.supabase_client.service_client.table('post_categories') \
                .select('id, name') \
                .eq('is_active', True) \
                .order('name') \
                .execute()
            ...
            blog_categories = [BlogCategory(id=item['id'], name=item['name']) for item in categories_data]
            return blog_categories

        except Exception as e:
            logger.error(f"Error fetching blog categories from database: {str(e)}")
            return []
