...
class MessagesService:
    def __init__(self, supabase_client):
        ...
        )  # Raw Supabase client for queries

    def _clean_message_response(
        ...
        if isinstance(message, dict):
            ...
            if "created_at" in message:
                message["created_at"] = format_datetime_central(
                    message["created_at"]
                )
            if "edited_at" in message:
                message["edited_at"] = format_datetime_central(
                    message["edited_at"]
                )
            if "updated_at" in message:
                message["updated_at"] = format_datetime_central(
                    message["updated_at"]
                )
        return message

    def _clean_conversation_response(
        self, conversation: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Format datetime fields in conversation response"""
        if isinstance(conversation, dict):
            if "created_at" in conversation:
                conversation["created_at"] = format_datetime_central(
                    conversation["created_at"]
                )
            if "updated_at" in conversation:
                conversation["updated_at"] = format_datetime_central(
                    conversation["updated_at"]
                )
            # Format last_message datetime if present
            if "last_message" in conversation and isinstance(
    ...
    async def get_user_conversations(
        ...
        try:
            logger.info(
                f"Getting conversations for user {user_id} with limit {limit}, offset {offset}"
            )
            ...
            if not participant_result.data:
                logger.info(
                    f"No conversation participants found for user {user_id}"
                )
                return []

            conversation_ids = [
                p["conversation_id"] for p in participant_result.data
            ]
            logger.info(
                f"User {user_id} participates in conversations: {conversation_ids}"
            )
            ...
            logger.info(
                f"Raw conversations query returned {len(conversations) if conversations else 0} results"
            )
            ...
            for conversation in conversations:
                ...
                active_participants = [
                    p for p in all_participants if p.get("left_at") is None
                ]
            ...
            if other_user_ids:
                users_result = (
                    self.supabase.table("user_signup_tracking")
            ...
            if other_user_ids:
                ...
                for profile_data in profiles_result.data or []:
                    if profile_data.get("avatar_url"):
                        avatars_map[profile_data["user_id"]] = profile_data[
                            "avatar_url"
                        ]
            ...
            if avatars_map:
                try:
                    ...
                    for uid, avatar_url in avatars_map.items():
                        signed_avatars_map[uid] = (
                            profile_service._generate_signed_avatar_url(
                                avatar_url
                            )
                        )
                except Exception as e:
                    logger.warning(
                        f"Failed to generate signed avatar URLs: {e}"
                    )

            # Get current user's podcast_id once
            current_user_podcast_id = (
                self.supabase_client.get_user_claimed_podcast_id(user_id)
            )
            ...
            for conversation in conversations:
                ...
                if messages:
                    ...
                    non_deleted_messages = [
                        m for m in messages if not m.get("is_deleted", False)
                    ]
                    if non_deleted_messages:
                        ...
                        if latest_message:
                            ...
                            latest_message = self._clean_message_response(
                                latest_message
                            )
                ...
                for participant in active_participants:
                    if participant["user_id"] != user_id:
                        ...
                        if other_user_id in users_info_map:
                            ...
                            display_info["name"] = user_name or (
                                user_email.split("@")[0]
                                if user_email
                                else "Unknown User"
                            )
                        ...
                        if other_user_id in signed_avatars_map:
                            display_info["image_url"] = signed_avatars_map[
                                other_user_id
                            ]
                ...
                conversation["current_user_podcast_id"] = (
                    current_user_podcast_id
                )
    ...
    async def create_conversation(
        self, creator_id: str, participant_ids: List[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(participant_ids) != 1:
                return {
                    "success": False,
                    "error": "Direct messages must be between exactly 2 users",
                }
            ...
            if not creator_ready.get("success") or not creator_ready.get(
                "is_ready"
            ):
            ...
            # Check recipient
            recipient_ready = supabase_client.is_user_platform_ready(
                recipient_id
            )
            if not recipient_ready.get("success") or not recipient_ready.get(
                "is_ready"
            ):
    ...
    async def get_messageable_users(
        ...
        try:
            ...
            user_conversation_ids = [
                c["conversation_id"]
                for c in (user_conversations_result.data or [])
            ]
            ...
            if user_conversation_ids:
                ...
                for conv_id, participant_ids in conv_participants.items():
                    ...
                    if (
                        ...
                        and user_id in participant_ids
                    ):
                        other_user_id = [
            ...
            for signup_user in result.data:
                ...
                user_info = {
                    ...
                    "last_name": last_name,
                    "display_name": (
                        profile.get("name") or name or email.split("@")[0]
                        if email
                        else "Unknown User"
                    ),
    ...
    async def delete_conversation(
        ...
        try:
            ...
            if result.data:
                logger.info(
                    f"User {user_id} left conversation {conversation_id}"
                )
                ...
                if not remaining_participants.data:
                    ...
                    logger.info(
                        f"Conversation {conversation_id} marked as inactive (no participants)"
                    )
        ...
        try:
            ...
            if before_message_id:
                ...
                if before_message.data:
                    query = query.lt(
                        "created_at", before_message.data["created_at"]
                    )
                ...
                if after_message.data:
                    query = query.gt(
                        "created_at", after_message.data["created_at"]
                    )

            result = (
                query.order("created_at", desc=True).limit(limit + 1).execute()
            )
            ...
            messages = await self._attach_reactions_to_messages(
                messages, user_id
            )
    ...
    async def send_message(
        ...
        media_files: Optional[
            List
        ] = None,  # New parameter for multiple media files
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not (
                has_content
                or has_media
                or has_legacy_attachment
                or has_voice
                or has_podcast_share
            ):
                return {
                    "success": False,
                    "error": "Message must have content, media, or other attachment",
                }
                ...
                message_data.update(
                    {
                        ...
                        "attachment_mime_type": attachment_data.get(
                            "mime_type"
                        ),
                    }
                )
            ...
            if voice_data:
                message_data.update(
                    {
                        "voice_duration_seconds": voice_data.get(
                            "duration_seconds"
                        ),
                        "voice_waveform": voice_data.get("waveform"),
                        "attachment_url": voice_data.get(
                            "audio_url"
                        ),  # Voice file URL
                    }
                )
            ...
            if podcast_share_data:
                message_data.update(
                    {
                        "shared_podcast_id": podcast_share_data.get(
                            "podcast_id"
                        ),
                        "shared_episode_id": podcast_share_data.get(
                            "episode_id"
                        ),
                    }
                )
            ...
            if result.data:
                ...
                if media_files and len(media_files) > 0:
                    ...
                    try:
                        uploaded_media = (
                            await media_service.upload_message_media(
                                user_id=sender_id,
                        ...
                        logger.info(
                            f"Uploaded {len(uploaded_media)} media files for message {message_id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to upload media for message {message_id}: {str(e)}"
                        )
                        # Continue even if media upload fails, message was created

                # Create delivery status entries for all participants (except sender)
                await self._create_delivery_status_entries(
                    conversation_id, message_id, sender_id
                )

                # Update user's last read timestamp
                await self._update_user_last_read(
                    conversation_id, sender_id, message_id
                )

                logger.info(
                    f"Message {message_id} sent in conversation {conversation_id}"
                )
    ...
    async def edit_message(
        ...
        try:
            ...
            if result.data:
                ...
                return {
                    "success": False,
                    "error": "Message not found, unauthorized, or already deleted",
                }
    ...
    async def update_user_online_status(
        ...
        try:
            ...
            result = (
                self.supabase.table("user_online_status")
        ...
        try:
            ...
            if up_to_message_id:
                ...
                if message_result.data:
                    ...
                    self.supabase.table("message_delivery_status").update(
    ...
    async def mark_message_as_read(
        ...
        try:
            ...
            if not update_result.data:
                self.supabase.table("message_delivery_status").upsert(
                    {
                        "message_id": message_id,
                        "user_id": user_id,
                        "delivered_at": now,
            ...
            # Update conversation participant's last read info
            self.supabase.table("conversation_participants").update(
                {"last_read_at": now, "last_read_message_id": message_id}
    ...
    async def mark_messages_as_read_up_to(
        self, conversation_id: str, user_id: str, up_to_message_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if messages_to_mark.data:
                ...
                self.supabase.table("message_delivery_status").update(
                    {"read_at": now}
                ).eq("user_id", user_id).in_("message_id", message_ids).is_(
                    "read_at", "null"
                ).execute()
            ...
            # Update conversation participant's last read info
            self.supabase.table("conversation_participants").update(
                {"last_read_at": now, "last_read_message_id": up_to_message_id}
            ).eq("conversation_id", conversation_id).eq(
                "user_id", user_id
            ).execute()

            logger.info(
                f"User {user_id} marked messages up to {up_to_message_id} as read in conversation {conversation_id}"
            )
            return {
                "success": True,
                "messages_marked": (
                    len(messages_to_mark.data) if messages_to_mark.data else 0
                ),
            }
    ...
    async def get_message_read_receipts(
        ...
        try:
            ...
            if receipts_result.data:
                ...
                user_ids = [
                    receipt["user_id"] for receipt in receipts_result.data
                ]
                ...
                if user_ids:
                    ...
                    if users_result.data:
                        for user in users_result.data:
                            user_names[user["user_id"]] = user.get(
                                "name"
    ...
    async def toggle_message_reaction(
        self, message_id: str, user_id: str, reaction_type: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not reaction_type or reaction_type.strip() == "":
                # Remove reaction
                result = (
                    self.supabase.table("message_reactions")
                    .delete()
                    .eq("message_id", message_id)
                    .eq("user_id", user_id)
                    .execute()
                ...
                logger.info(
                    f"User {user_id} removed reaction from message {message_id}"
                )
            ...
            if reaction_type not in allowed_reactions:
                return {
                    "success": False,
                    "error": f"Invalid reaction type. Allowed: {', '.join(sorted(allowed_reactions))}",
                }
            ...
            if result.data:
                logger.info(
                    f"User {user_id} set reaction '{reaction_type}' on message {message_id}"
                )
        ...
        try:
            ...
            if reactions_result.data:
                for reaction in reactions_result.data:
                    ...
                    reactions_grouped[reaction_type]["users"].append(
                        reactor_id
                    )
            ...
            if conversation_id:
                ...
                if user_conversations.data:
                    ...
                    if len(conversation_ids) == 1:
                        ...
                        single_builder = single_builder.order(
                            "created_at", desc=True
                        )
                        single_builder = single_builder.range(
                        ...
                        for conv_id in conversation_ids:
                            try:
                                ...
                                conv_builder = conv_builder.order(
                                    "created_at", desc=True
                                )
                                ...
                                logger.error(
                                    f"Error searching conversation {conv_id}: {e}"
                                )
                                continue
                        ...
                        # Attach reactions to messages
                        paginated_messages = (
                            await self._attach_reactions_to_messages(
                                paginated_messages, user_id
                            )
                        )

                        # Clean up message responses
                        paginated_messages = self._clean_messages_response(
                            paginated_messages
                        )
    ...
    async def _get_conversation_display_info(
        self, conversation: Dict[str, Any], user_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if participants_result.data:
                ...
                display_info["user_id"] = (
                    other_user_id  # Add the other user's user_id
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to get avatar for user {other_user_id}: {e}"
                    )
    ...
    async def _get_users_online_status(
    ...
    async def _attach_media_to_messages(
        ...
        try:
            ...
            media_result = (
                self.supabase.table("message_media")
                .select("*")
            ...
            for media_item in media_result.data or []:
                ...
                media_item["url"] = media_service._generate_signed_url(
                ...
                if media_item.get("thumbnail_path"):
                    media_item["thumbnail_url"] = (
                        media_service._generate_signed_url(
    ...
    async def _attach_reactions_to_messages(
        self, messages: List[Dict[str, Any]], user_id: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for reaction in reactions_result.data or []:
                ...
                reactions_by_message[msg_id][reaction_type]["users"].append(
                    reactor_id
                )
    ...
    async def _create_delivery_status_entries(
        ...
        try:
            ...
            if participants_result.data:
                ...
                self.supabase.table("message_delivery_status").insert(
                    delivery_entries
                ).execute()
    ...
    async def _update_user_last_read(
