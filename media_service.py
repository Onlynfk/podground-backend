...
class MediaService:
        ...
        try:
            for file in files:
                ...
                storage_path = await self._generate_storage_path(
                    user_id, file.filename, file_info["type"]
                )
    ...
    async def _validate_file(self, file: UploadFile) -> Dict:
        ...
        if size > max_size:
            ...
            raise HTTPException(
                400, f"File too large. Maximum size: {max_mb}MB"
            )

        return {"type": file_type, "mime_type": mime_type, "size": size}

    async def _generate_storage_path(
        self, user_id: str, filename: str, file_type: str
    ) -> str:
    ...
    async def _process_media_file(
        self, file_content: bytes, file_info: Dict, storage_path: str
    ) -> Dict:
        ...
        try:
            if file_info["type"] == "image":
                ...
                processed_data = await self._process_video(
    ...
    async def _process_video(
        self, video_content: bytes, storage_path: str
    ) -> Dict:
    ...
    async def _upload_to_storage(
        self, file_content: bytes, storage_path: str, mime_type: str
    ) -> str:
        ...
        try:
            ...
            if os.getenv("ENVIRONMENT") == "dev":
                logger.debug(
                    f"Upload debug - first 8 bytes: {file_content[:8].hex()}"
                )
            ...
            if os.getenv("ENVIRONMENT") == "dev" and mime_type.startswith(
                "image/"
            ):
    ...
    async def _verify_upload_integrity(
        self, original_content: bytes, public_url: str
    ):
            ...
            if response.status_code == 200:
                ...
                if len(uploaded_content) == len(original_content):
                    if uploaded_content[:8] == original_content[:8]:
                        ...
                        logger.error(
                            f"❌ Upload corruption detected - signature mismatch"
                        )
                        logger.error(f"Original: {original_content[:8].hex()}")
                        logger.error(f"Uploaded: {uploaded_content[:8].hex()}")
                else:
                    logger.error(
                        f"❌ Upload size mismatch: {len(original_content)} vs {len(uploaded_content)}"
                    )
        except Exception as e:
    ...
    async def _store_temp_media(self, media_data: Dict) -> Dict:
        ...
        try:
            result = (
                self.supabase_client.service_client.table("temp_media_uploads")
                .insert(media_data)
                .execute()
            )
    ...
    async def mark_media_as_used(
        self, media_urls: List[str], user_id: str
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Marked {len(media_urls)} media files as used for user {user_id}"
            )
            return True
        except Exception as e:
    ...
    def generate_signed_url(
        self, storage_path: str, expiry: int = 3600, bucket: str = None
    ) -> str:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to generate signed URL for {storage_path}: {str(e)}"
            )
            raise
