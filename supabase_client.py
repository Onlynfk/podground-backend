...
class SupabaseClient:
    def __init__(self):
        ...
        self.service_key = os.getenv("SUPABASE_SERVICE_KEY")  # Keep for admin operations only
        
        if not self.url or not self.anon_key:
            logger.warning("Supabase URL or anon key not found in environment variables")
            self.client = None
    ...
    def _get_headers(self, user_token: str = None, use_service_key: bool = False):
    ...
    def _make_request(self, method: str, endpoint: str, data: Dict = None, user_token: str = None, use_service_key: bool = False) -> Dict:
    ...
    def sign_up_user_with_magic_link(self, email: str, name: str, redirect_url: str) -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": f"Magic link sign up failed: {str(e)}"}
    ...
    def create_user_without_email(self, email: str, name: str) -> Dict:
        ...
        if not self.service_client:
            return {"success": False, "error": "Supabase service client not initialized"}
    ...
    def generate_magic_link(self, email: str, frontend_callback_url: str, expiry_seconds: int = None) -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": f"Magic link generation failed: {str(e)}"}
    
    def generate_short_verification_code(self, user_id: str, length: int = 6) -> str:
        ...
        try:
            self.service_client.auth.admin.update_user_by_id(
                user_id,
                {"user_metadata": {"verification_code": code, "code_expires_at": "now() + interval '24 hours'"}}
            )
    ...
    def get_user_id_by_email(self, email: str) -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to get user_id by email: {str(e)}"}
    ...
    def verify_short_code(self, email: str, code: str) -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": f"Code verification failed: {str(e)}"}
    
    def sign_in_user_with_magic_link(self, email: str, redirect_url: str, expiry_seconds: int = None) -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": f"Magic link sign in failed: {str(e)}"}
    
    def verify_magic_link_token(self, token: str, type: str = "magiclink") -> Dict:
        ...
        try:
            ...
            return {"success": False, "error": f"Magic link verification failed: {str(e)}"}
    ...
    def exchange_code_for_session(self, code: str) -> Dict:
        ...
        if clean_code.startswith('eyJ') and len(clean_code) > 100:
            ...
            try:
                logger.info(f"Attempting to validate JWT access token (length: {len(clean_code)})")
                ...
                try:
                    ...
                    decoded_token = jwt.decode(clean_code, options={"verify_signature": False})
                    ...
                    if user_id and email:
                        ...
                        logger.warning("JWT token missing required user information")
        ...
        try:
            ...
            try:
                ...
                logger.error(f"OTP verification fallback also failed: {str(e2)}")
                return {"success": False, "error": f"All authentication methods failed. PKCE: {str(e)}, OTP: {str(e2)}"}
    
    # Profile methods
    def create_user_profile(self, user_id: str, first_name: str, last_name: str, user_token: str) -> Dict:
        """Create user profile"""
        data = {
            "id": user_id,
            "first_name": first_name,
        ...
        return self._make_request("POST", "user_profiles", data, user_token=user_token)
    
    def get_user_profile(self, user_id: str, user_token: str) -> Dict:
        """Get user profile"""
        return self._make_request("GET", f"user_profiles?id=eq.{user_id}", user_token=user_token)
    
    # Onboarding methods
    def save_onboarding_data(self, user_id: str, onboarding_data: Dict, user_token: str = None) -> Dict:
        ...
        if user_token is None:
            try:
                ...
                result = self.service_client.table("user_onboarding").insert(data).execute()
                
                if result.data:
                    # Insert categories into junction table
                    categories_result = self._save_user_categories(user_id, category_ids)
                    
                    if not categories_result["success"]:
                        # Rollback main insert if categories failed
                        self.service_client.table("user_onboarding").delete().eq("id", user_id).execute()
                        return {"success": False, "error": f"Failed to save categories: {categories_result.get('error')}"}
                    
                    return {"success": True, "data": result.data}
                else:
                    return {"success": False, "error": "Failed to save onboarding data"}
            except Exception as e:
                logger.error(f"Failed to save onboarding data via service client: {str(e)}")
                return {"success": False, "error": str(e)}
        else:
            # Use user token for JWT-based auth (legacy)
            return self._make_request("POST", "user_onboarding", data, user_token=user_token)
    
    def get_onboarding_data(self, user_id: str, user_token: str = None) -> Dict:
        ...
        if user_token is None:
            try:
                ...
                result = self.service_client.table("user_onboarding").select("*").eq("id", user_id).execute()
                
                if result.data is not None:
                    return {"success": True, "data": result.data}
                else:
                    return {"success": False, "error": "Failed to get onboarding data"}
            except Exception as e:
                logger.error(f"Failed to get onboarding data via service client: {str(e)}")
                return {"success": False, "error": str(e)}
        else:
            # Use user token for JWT-based auth (legacy)
            return self._make_request("GET", f"user_onboarding?id=eq.{user_id}", user_token=user_token)
    
    def _save_user_categories(self, user_id: str, category_ids: List[str]) -> Dict:
        ...
        try:
            ...
            self.service_client.table("user_onboarding_categories").delete().eq("user_id", user_id).execute()
            ...
            # Debug logging
            logger.info(f"DEBUG: Inserting category records: {category_records}")
            logger.info(f"DEBUG: user_id type: {type(user_id)}, category_ids types: {[type(cid) for cid in category_ids]}")
            
            result = self.service_client.table("user_onboarding_categories").insert(category_records).execute()
    ...
    def save_onboarding_step(self, user_id: str, step: int, step_data: Dict, user_token: str = None, automatic_favorites: List[Dict] = None) -> Dict:
        ...
        if step == 1:
            ...
            update_data.update({
                "podcasting_experience": step_data.get("podcasting_experience"),
            ...
            update_data.update({
                "is_part_of_network": step_data.get("is_part_of_network", False),
            ...
            if not location_id:
                return {"success": False, "error": "location_id is required for step 3"}
            ...
            update_data.update({
                "looking_for_guests": step_data.get("looking_for_guests", False),
                "wants_to_be_guest": step_data.get("wants_to_be_guest", False),
        ...
        if not existing["success"] or not existing["data"]:
            ...
            if user_token is None:
                ...
                try:
                    result = self.service_client.table("user_onboarding").insert(create_data).execute()
                    if result.data:
                        return {"success": True, "data": result.data}
                    else:
                        return {"success": False, "error": "Failed to create onboarding record"}
                except Exception as e:
                    logger.error(f"Failed to create onboarding step via service client: {str(e)}")
                    return {"success": False, "error": str(e)}
            else:
                # Use JWT-based auth (legacy)
                return self._make_request("POST", "user_onboarding", create_data, user_token=user_token)
        else:
            # Update existing record
            if user_token is None:
                # Use service client for session-based auth
                try:
                    result = self.service_client.table("user_onboarding").update(update_data).eq("id", user_id).execute()
                    if result.data:
                        return {"success": True, "data": result.data}
                    else:
                        return {"success": False, "error": "Failed to update onboarding record"}
                except Exception as e:
                    logger.error(f"Failed to update onboarding step via service client: {str(e)}")
                    return {"success": False, "error": str(e)}
            else:
                # Use JWT-based auth (legacy)
                return self._make_request("PATCH", f"user_onboarding?id=eq.{user_id}", update_data, user_token=user_token)
    
    def mark_podcast_claim_completed(self, user_id: str, user_token: str, podcast_title: str = None) -> Dict:
    ...
    def create_podcast_claim(self, user_id: str, listennotes_id: str, podcast_title: str, 
                           user_token: str = None, podcast_email: str = None, verification_code: str = None,
        ...
        if user_token is None:
            try:
                ...
                result = self.service_client.table("podcast_claims").insert(data).execute()
                
                if result.data:
                    return {"success": True, "data": result.data, "claim_id": result.data[0]["id"]}
                else:
                    return {"success": False, "error": "Failed to create podcast claim"}
            except Exception as e:
                logger.error(f"Failed to create podcast claim via service client: {str(e)}")
                return {"success": False, "error": str(e)}
        else:
            # Use user token for JWT-based auth (legacy)
            return self._make_request("POST", "podcast_claims", data, user_token=user_token)
    
    def verify_podcast_claim(self, claim_id: str, verification_code: str, user_token: str) -> Dict:
        """Verify podcast claim with code"""
        # First get the claim - this will respect RLS and only return user's own claims
        claim_result = self._make_request("GET", f"podcast_claims?id=eq.{claim_id}", user_token=user_token)
        ...
        if claim.get("verification_code_expires_at"):
            from datetime import datetime, timezone
            expiry_time = datetime.fromisoformat(claim["verification_code_expires_at"].replace('Z', '+00:00'))
        ...
        return self._make_request("PATCH", f"podcast_claims?id=eq.{claim_id}", update_data, user_token=user_token)
    
    def verify_podcast_claim_by_email(self, user_id: str, verification_code: str) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("podcast_claims").select("*").eq(
                "user_id", user_id
            ).eq("verification_code", verification_code).eq(
                "is_verified", False
            ).eq("claim_status", "pending").order("created_at", desc=True).limit(1).execute()
            
            if not result.data:
                return {"success": False, "error": "Invalid verification code or no pending claims found"}
            ...
            # Update claim as verified
            update_result = self.service_client.table("podcast_claims").update({
                "is_verified": True,
                "claim_status": "verified",
                "verified_at": "now()"
            ...
            if update_result.data:
                try:
                    ...
                    logger.info(f"Successfully imported claimed podcast '{claim.get('podcast_title')}' to main podcasts table")
                except Exception as import_error:
                    logger.error(f"Failed to import claimed podcast to main table: {import_error}")
    ...
    def get_user_podcast_claims(self, user_id: str, user_token: str) -> Dict:
        ...
        return self._make_request("GET", f"podcast_claims?user_id=eq.{user_id}", user_token=user_token)
    ...
    def get_user_podcast_claims_session(self, user_id: str) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("podcast_claims").select("*").eq("user_id", user_id).execute()
            
            if result.data is not None:
                return {"success": True, "data": result.data}
            else:
                return {"success": False, "error": "Failed to get podcast claims"}
        except Exception as e:
            logger.error(f"Failed to get podcast claims via service client: {str(e)}")
            return {"success": False, "error": str(e)}
    ...
    def get_user_claimed_podcast_id(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            claims_result = self.service_client.table("podcast_claims") \
            ...
            podcast_result = self.service_client.table("podcasts") \
                .select("id") \
    ...
    def check_podcast_already_claimed(self, listennotes_id: str, current_user_id: str = None) -> Dict:
    ...
    def check_user_signup_confirmed(self, user_id: str) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("user_signup_tracking").select("*").eq(
                "user_id", user_id
            ).eq("signup_confirmed", True).execute()
    ...
    def create_signup_tracking(self, user_id: str, email: str, name: str = None) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("user_signup_tracking").insert(tracking_data).execute()
    ...
    def get_signup_tracking_by_user_id(self, user_id: str) -> Dict:
        ...
        try:
            result = self.service_client.table("user_signup_tracking").select("*").eq("user_id", user_id).execute()
    ...
    def mark_signup_confirmed(self, user_id: str) -> Dict:
        ...
        try:
            ...
            existing = self.service_client.table("user_signup_tracking").select("*").eq("user_id", user_id).execute()
            ...
            logger.info(f"User {user_id} current signup_confirmed value: {current_confirmed} (type: {type(current_confirmed)})")
            
            # If already confirmed, don't update
            if current_confirmed == True:
                logger.info(f"User {user_id} already marked as signup confirmed")
                return {"success": True, "message": "User already confirmed"}
            
            # Update only if signup_confirmed is False (not True)
            result = self.service_client.table("user_signup_tracking").update({
                "signup_confirmed": True,
                "signup_confirmed_at": "now()"
            }).eq("user_id", user_id).eq("signup_confirmed", False).execute()
            
            logger.info(f"Update result for user {user_id}: {result.data}")
            
            if result.data:
                logger.info(f"Successfully marked signup confirmed for user {user_id}")
                ...
                check_again = self.service_client.table("user_signup_tracking").select("*").eq("user_id", user_id).execute()
                if check_again.data:
                    record_after = check_again.data[0]
                    logger.error(f"Update failed for user {user_id}. Record after attempt: {record_after}")
                else:
    ...
    def mark_first_login(self, user_id: str) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("user_signup_tracking").update({
                "has_logged_in": True,
                "first_login_at": "now()"
            }).eq("user_id", user_id).eq("has_logged_in", False).execute()
            ...
            if result.data:
                ...
                if existing.data and existing.data[0].get("has_logged_in"):
                    ...
                    return {"success": True, "message": "User already logged in"}
                else:
                    logger.warning(f"No tracking record found for user {user_id}")
                    return {"success": False, "error": "No tracking record found"}
    ...
    def get_users_needing_reminder(self, hours_since_signup: int = 24) -> Dict:
        ...
        try:
            ...
            from datetime import datetime, timedelta, timezone
            cutoff_time = datetime.now(timezone.utc) - timedelta(hours=hours_since_signup)
            cutoff_iso = cutoff_time.isoformat()
            
            result = self.service_client.table("user_signup_tracking").select("*").lt(
                "signup_at", cutoff_iso
            ).eq("has_logged_in", False).eq("reminder_sent", False).execute()
            
            if result.data is not None:
                logger.info(f"Found {len(result.data)} users needing reminders")
                return {"success": True, "data": result.data}
            else:
    ...
    def mark_reminder_sent(self, user_id: str) -> Dict:
        ...
        try:
            ...
            if result.data:
                ...
                logger.error(f"Failed to mark reminder sent for user {user_id}")
    ...
    def get_users_needing_confirmation_sync(self) -> Dict:
        ...
        try:
            result = self.service_client.table("user_signup_tracking").select("*").eq(
                "signup_confirmed", True
            ).eq("confirmation_synced_to_customerio", False).execute()
            
            if result.data is not None:
                logger.info(f"Found {len(result.data)} users needing confirmation sync")
                return {"success": True, "data": result.data}
            else:
                logger.error(f"Failed to get users needing confirmation sync: {result}")
                return {"success": False, "error": "Failed to query users"}
                
        except Exception as e:
            logger.error(f"Error getting users needing confirmation sync: {str(e)}")
            return {"success": False, "error": str(e)}
    ...
    def mark_confirmation_synced_to_customerio(self, user_id: str) -> Dict:
        ...
        try:
            ...
            if result.data:
                logger.info(f"Marked confirmation synced to Customer.io for user {user_id}")
                return {"success": True, "data": result.data}
            else:
                logger.error(f"Failed to mark confirmation synced for user {user_id}")
    ...
    def get_signup_stats(self) -> Dict:
        ...
        try:
            # Get all records
            all_users = self.service_client.table("user_signup_tracking").select("*").execute()
            
            if all_users.data is None:
                return {"success": False, "error": "Failed to fetch stats"}
            
            total_signups = len(all_users.data)
            users_logged_in = len([u for u in all_users.data if u.get("has_logged_in")])
            users_not_logged_in = total_signups - users_logged_in
            reminders_sent = len([u for u in all_users.data if u.get("reminder_sent")])
            ...
            for user in all_users.data:
                signup_time = datetime.fromisoformat(user["signup_at"].replace('Z', '+00:00'))
                if (signup_time <= cutoff_time and 
                    not user.get("has_logged_in") and 
                    not user.get("reminder_sent")):
    ...
    def get_user_subscription(self, user_id: str) -> Dict:
        ...
        try:
            ...
            if result.data:
                ...
                plan_result = self.service_client.table("subscription_plans").select("*").eq(
    ...
    def create_subscription(self, user_id: str, plan_name: str, stripe_customer_id: str = None, stripe_subscription_id: str = None) -> Dict:
        ...
        try:
            ...
            if plan_name != "free":
                subscription_data["ends_at"] = (datetime.now(timezone.utc) + timedelta(days=30)).isoformat()
            
            # Delete existing subscription first (since we have UNIQUE constraint)
            self.service_client.table("user_subscriptions").delete().eq("user_id", user_id).execute()
            
            # Insert new subscription
            result = self.service_client.table("user_subscriptions").insert(subscription_data).execute()
    ...
    def update_subscription_status(self, user_id: str, status: str) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("user_subscriptions").update(update_data).eq(
                "user_id", user_id
            ).execute()
    ...
    def get_subscription_plans(self) -> Dict:
        ...
        try:
            result = self.service_client.table("subscription_plans").select("*").eq(
                "is_active", True
    ...
    def ensure_user_defaults(self, user_id: str) -> Dict:
        ...
        try:
            ...
            role_result = self.service_client.table("user_roles") \
                .select("role") \
                .eq("user_id", user_id) \
                .execute()
            ...
            if not role_result.data:
                ...
                role_assign_result = self.assign_user_role(user_id, "podcaster", granted_by=None)
                if not role_assign_result["success"]:
                    logger.error(f"Failed to assign default role to user {user_id}")
            
            # Check and assign default subscription if missing
            subscription_result = self.service_client.table("user_subscriptions") \
                .select("plan_id") \
                .eq("user_id", user_id) \
                .execute()
            
            if not subscription_result.data:
                logger.info(f"Assigning default subscription to user {user_id}")
                subscription_assign_result = self.create_subscription(user_id, "free")
                if not subscription_assign_result["success"]:
                    logger.error(f"Failed to assign default subscription to user {user_id}")
    ...
    def assign_user_role(self, user_id: str, role: str, granted_by: str = None) -> Dict:
        ...
        try:
            ...
            self.service_client.table("user_roles").delete().eq("user_id", user_id).execute()
            
            # Insert new role
            result = self.service_client.table("user_roles").insert(role_data).execute()
    ...
    def get_podcast_claims_needing_reminder(self, hours_since_created: int = 24) -> Dict:
        ...
        try:
            # Calculate cutoff time
            from datetime import datetime, timedelta, timezone
            cutoff_time = datetime.now(timezone.utc) - timedelta(hours=hours_since_created)
            cutoff_iso = cutoff_time.isoformat()
            
            result = self.service_client.table("podcast_claims").select("*").lt(
                "created_at", cutoff_iso
            ).eq("is_verified", False).eq("reminder_sent", False).eq("claim_status", "pending").execute()
            
            if result.data is not None:
                logger.info(f"Found {len(result.data)} podcast claims needing reminders")
                return {"success": True, "data": result.data}
            else:
                logger.error(f"Failed to get claims needing reminder: {result}")
    ...
    def mark_podcast_claim_reminder_sent(self, claim_id: str) -> Dict:
        ...
        try:
            result = self.service_client.table("podcast_claims").update({
                "reminder_sent": True,
                "reminder_sent_at": "now()"
            ...
            if result.data:
                ...
                logger.error(f"Failed to mark reminder sent for claim {claim_id}")
                return {"success": False, "error": "Failed to update reminder status"}
    ...
    def update_podcast_claim_verification_code(self, claim_id: str, new_code: str, expiry_hours: int = 24) -> Dict:
        ...
        try:
            ...
            if result.data:
                ...
                logger.error(f"Failed to update verification code for claim {claim_id}")
    ...
    def get_active_podcast_categories(self) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("podcast_categories").select("*").eq("is_active", True).order("sort_order").execute()
            
            if result.data is not None:
                logger.info(f"Retrieved {len(result.data)} active podcast categories")
                return {"success": True, "data": result.data}
            else:
    ...
    def get_states_countries(self, country_code: str = None, search: str = None, limit: int = 100, offset: int = 0) -> Dict:
        ...
        try:
            ...
            if search:
                ...
                query = query.or_(f"name.ilike.{search_term},country_name.ilike.{search_term}")
            
            # Apply pagination and ordering (max 1000 records due to Supabase limit)
            query = query.order("country_name").order("name").range(offset, offset + limit - 1)
            ...
            if result.data is not None:
                ...
                return {"success": False, "error": "Failed to fetch states and countries"}
...
# Convert all enums to their string values
    ...
    def add_waitlist_email(self, email: str, first_name: str, last_name: str, variant: str) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("waitlist_emails").insert(data).execute()
    ...
    def add_microgrant_waitlist_email(self, email: str, first_name: str, last_name: str) -> Dict:
        ...
        try:
            ...
            result = self.service_client.table("microgrant_waitlist_emails").insert(data).execute()
            
            if result.data:
                logger.info(f"Added email {email} to Supabase microgrant waitlist")
                return {"success": True, "data": result.data[0]}
            else:
                return {"success": False, "error": "Failed to add email to microgrant waitlist"}
                
        except Exception as e:
            logger.error(f"Failed to add microgrant waitlist email to Supabase: {str(e)}")
            if "duplicate key" in str(e).lower():
                return {"success": False, "error": "Email already exists in microgrant waitlist"}
            return {"success": False, "error": str(e)}
    
    def update_customerio_sync_status(self, table_name: str, record_id: int, status: str, increment_attempts: bool = False) -> Dict:
        ...
        try:
            ...
            if increment_attempts:
                ...
                current_record = self.service_client.table(table_name).select("customerio_sync_attempts").eq("id", record_id).execute()
                if current_record.data:
                    current_attempts = current_record.data[0].get("customerio_sync_attempts", 0)
                    update_data["customerio_sync_attempts"] = current_attempts + 1
            
            result = self.service_client.table(table_name).update(update_data).eq("id", record_id).execute()
    ...
    def get_unsynced_waitlist_entries(self, table_name: str = "waitlist_emails", retry_limit: int = 5) -> Dict:
        """Get waitlist entries that need Customer.io sync"""
        if not self.service_client:
            return {"success": False, "error": "Service client not initialized"}
        
        try:
            result = self.service_client.table(table_name).select("*").in_(
                "customerio_sync_status", ["pending", "failed"]
            ).lt("customerio_sync_attempts", retry_limit).order("created_at").execute()
            
            if result.data is not None:
                return {"success": True, "data": result.data}
            else:
                return {"success": False, "error": "Failed to get unsynced entries"}
        ...
        try:
            ...
            for podcast_id in favorite_podcast_ids[:5]:  # Only take first 5
                ...
                if valid_id:
                    ...
                    logger.warning(f"Skipping invalid podcast ID from favorites: {podcast_id}")
                ...
                for auto_fav in automatic_favorites:
                    if auto_fav.get("podcast_id"):
                        ...
                        if auto_fav["podcast_id"] not in favorite_podcast_ids:
                            valid_id = get_valid_podcast_id(auto_fav["podcast_id"])
                            if valid_id and valid_id not in valid_podcast_ids:
                                ...
                                logger.info(f"Adding auto-favorite podcast: {auto_fav.get('podcast_title', valid_id)}")
                            elif not valid_id:
                                logger.warning(f"Skipping invalid auto-favorite podcast ID: {auto_fav['podcast_id']} ({auto_fav.get('podcast_title', 'Unknown')})")
            
            if follows_data:
                # First, delete any existing follows for this user to avoid duplicates
                self.service_client.table("user_podcast_follows").delete().eq("user_id", user_id).execute()
                
                # Insert the new follows
                result = self.service_client.table("user_podcast_follows").insert(follows_data).execute()
                
                if result.data:
                    logger.info(f"Created {len(follows_data)} podcast follows for user {user_id}")
                    return {"success": True, "data": result.data}
                else:
                    return {"success": False, "error": "Failed to create follows"}
            else:
                logger.warning(f"No valid podcast IDs found to create follows for user {user_id}")
    ...
    def _import_claimed_podcast_to_main_table(self, claim: Dict) -> bool:
        ...
        try:
            ...
            existing_check = self.service_client.table("podcasts").select("id").eq("listennotes_id", listennotes_id).execute()
            if existing_check.data:
                logger.info(f"Podcast with ListenNotes ID {listennotes_id} already exists in main table")
                return True
            ...
            try:
                ...
                logger.warning(f"Could not fetch full data from ListenNotes: {ln_error}, using minimal data")
            ...
            result = self.service_client.table("podcasts").insert(insert_data).execute()
            ...
            if result.data:
                ...
                logger.error(f"Failed to insert claimed podcast '{podcast_title}' into main table")
                return False
