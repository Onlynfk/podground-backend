...
class ResourcesService:
    def __init__(self):
        ...
        if not self.partners_bucket:
            raise ValueError("R2_PARTNERS_BUCKET_NAME environment variable is required")

        self.resources_bucket = os.getenv('R2_RESOURCES_BUCKET_NAME')
        if not self.resources_bucket:
            raise ValueError("R2_RESOURCES_BUCKET_NAME environment variable is required")
        ...
        try:
            ...
            if response.data:
                ...
                for resource in response.data:
                    ...
                    resource_data = {k: v for k, v in resource.items() 
                    ...
                    if resource.get('is_premium') and not user_is_premium:
                        ...
                        if resource.get('type') == 'video':
                            ...
                            resource_data['description'] = f"🔒 {resource_data['description'][:100]}... [Premium content - upgrade to access]"
                    
                    # Add content URLs for articles
                    if resource.get('type') in ['article', 'guide']:
                        resource_data['content_url'] = await article_content_service.generate_article_url(resource['id'])
                        resource_data['content_api_url'] = f"/api/v1/resources/{resource['id']}/content"
                    
                    # Add PDF guide download URL for articles and videos
                    if resource.get('type') in ['article', 'video'] and resource.get('download_url'):
                        # For articles, use action guide service (existing functionality)
                        if resource.get('type') == 'article':
                            download_url = action_guide_service.generate_download_url(
                                resource['id'],
                                resource.get('category', 'general'),
                            )
                            if download_url:
                                resource_data['action_guide_url'] = download_url
                            else:
                                resource_data['action_guide_url'] = resource.get('download_url')
                        
                        # For videos, use the new PDF service
                        elif resource.get('type') == 'video':
                            from resource_pdf_service import resource_pdf_service
                            download_url = resource_pdf_service.generate_download_url(
                                resource['id'],
                                'video'
                            )
                            if download_url:
                                ...
                                resource_data['pdf_guide_url'] = resource.get('download_url')
                    
                    if resource_data.get("user_has_access", False):
                        for field in ("image_url", "thumbnail_url", "url", "download_url"):
                            val = resource_data.get(field)
                            if val:
                                resource_data[field] = self._generate_signed_url_from_r2_url(val)
    ...
    async def get_resource_by_id(self, user_id: str, resource_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response.data:
                ...
                # Remove unwanted fields from response
                resource = {k: v for k, v in resource_raw.items() 
                           if k not in ['duration', 'difficulty_level', 'subcategory']}
                ...
                if resource.get('type') in ['article', 'guide']:
                    ...
                    resource['content_url'] = await article_content_service.generate_article_url(resource_id)
                    
                    # Also add an API endpoint for fetching content through our API
                    resource['content_api_url'] = f"/api/v1/resources/{resource_id}/content"
                    
                    # Check if content exists
                    metadata = await article_content_service.get_article_metadata(resource_id)
                ...
                if resource.get('type') in ['article', 'video'] and resource.get('download_url'):
                    ...
                    if resource.get('type') == 'article':
                        ...
                        if download_url:
                            ...
                            resource['action_guide_url'] = resource.get('download_url')
                    ...
                    # For videos, use the new PDF service
                        ...
                        if download_url:
                            ...
                            resource['pdf_guide_url'] = resource.get('download_url')
                
                if resource.get("user_has_access", False):
                    for field in ("image_url", "thumbnail_url", "url",):
                        val = resource.get(field)
                        if val:
                            resource[field] = self._generate_signed_url_from_r2_url(val)
    ...
    async def get_resource_categories(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.supabase.table('resource_categories').select('*').eq('is_active', True).order('sort_order').execute()
            return response.data or []
        except Exception as e:
            logger.error(f"Error getting resource categories: {str(e)}")
            # Fallback to static categories if database query fails
            return [
                {'name': 'planning', 'display_name': 'Planning & Strategy', 'description': 'Content planning and strategic guidance'},
                {'name': 'production', 'display_name': 'Production & Editing', 'description': 'Recording and editing techniques'},
                {'name': 'promotion', 'display_name': 'Promotion & Growth', 'description': 'Marketing and audience building'},
                {'name': 'monetization', 'display_name': 'Monetization', 'description': 'Revenue streams and business models'},
                {'name': 'equipment', 'display_name': 'Equipment & Tools', 'description': 'Microphones, software, and setups'}
            ]
    ...
    async def get_experts(
        ...
        try:
            ...
            query = query.order('rating', desc=True).order('is_available', desc=True)
            ...
            if response.data:
                ...
                for expert in experts:
                    if expert.get('avatar_url'):
                        expert['avatar_url'] = self._generate_signed_url_from_r2_url(expert['avatar_url'])
    ...
    def update_resource_download_url(self, resource_id: str, download_url: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.supabase.table('resources').update(
                update_data
    ...
    async def get_expert_by_id(self, expert_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Get expert data
            response = self.supabase.table('experts').select('*').eq('id', expert_id).execute()
    ...
    async def get_partner_deals(
        ...
        try:
            ...
            query = self.supabase.table('partner_deals').select('*').eq('is_active', True)
                ...
                for deal in response.data:
                    ...
                    if deal.get('image_url'):
                        try:
                            ...
                            logger.warning(f"Failed to generate signed URL for {deal['partner_name']}: {e}")
                            deal['logo_url'] = None
    ...
    async def get_partner_deal_by_id(self, deal_id: str) -> Optional[Dict[str, Any]]:
        """Get single partner deal by ID"""
        try:
            from media_service import MediaService
            media_service = MediaService()

            response = self.supabase.table('partner_deals').select('*').eq('id', deal_id).eq('is_active', True).execute()
    ...
    async def get_article_content(self, user_id: str, resource_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if resource.get('type') != 'article':
                return {'success': False, 'error': 'Resource is not an article'}
            
            # Fetch content from R2
            content = await article_content_service.get_article_content(resource_id)
    ...
    async def create_resource(self, resource_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.supabase.table('resources').insert(insert_data).execute()
            
            if response.data:
                created_resource = response.data[0]
                # Apply field filtering to response
                filtered_resource = {k: v for k, v in created_resource.items() 
                                   if k not in ['duration', 'difficulty_level', 'subcategory']}
    ...
    async def _increment_view_count(self, resource_id: str):
        ...
        try:
            ...
            current = self.supabase.table('resources').select('view_count').eq('id', resource_id).execute()
            if current.data:
            ...
            logger.error(f"Error incrementing view count for resource {resource_id}: {str(e)}")
